...
- **`@/storage` and `@/hooks` barrels ARE client-safe** — unlike `@/utils` and component-group barrels (see above), they re-export no fs loaders, so `'use client'` components import them via the barrel, not a direct path. The direct-path rule applies only to barrels that drag `node:fs` into the client bundle.
